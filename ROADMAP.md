...
## 9. Web research (keyless, beta.26)

- [x] **Keyless web search** (`web_search.py`, W1) — DuckDuckGo HTML endpoint with a browser User-Agent and `uddg=` redirect-unwrapping; no API keys, provider-fallback chain. Pure `parse_ddg_html` / `_decode_ddg_url` unit-tested; live-verified.
- [x] **Browser-imitation fetch + main-text extraction** (`web_fetch.py`, W2) — bs4 heuristic strips nav/header/footer/aside/script/style and picks the article/main/body container; web PDFs reuse the existing `_read_pdf`. `fetch_safe()`→None so one bad link can't kill a run.
- [x] **Deep-research orchestration** (`deep_research.py`, W4) — search → fetch top-N → **grounded synthesis with `[N]`→URL citations**; `deep=True` map-reduce extracts per-page question-relevant facts (concurrent, empty-note filtered) then reduces into one cited answer, covering many sources without overflowing the model window. Pure message-builders unit-tested; live-verified (RAG-vs-fine-tuning → 6/6 sources cited with a synthesized comparison table).
- [x] **Web importers into notebooks** (`web_import.py`, W3) — keyless import of a **Wikipedia** article (extracts API, descriptive UA — a fake browser UA gets 403) and a public **GitLab** repo's text/code files (public REST API, URL-encoded project path), both reusing `Notebook.add_url_source`. Confluence intentionally has no dedicated parser — a Confluence page is just a web page (covered by `＋ URL` / `🌐 Веб-поиск`); a parser would only matter for auth-gated spaces.
- [x] **UI: `🌐 Веб-поиск` chat mode + importer buttons** (`notebook_gui.py`, W5) — a chat toggle routes the question through deep research instead of the notebook index (no index required); progress streams into the bubble; web sources render as the same `[N]` chips whose click opens the URL in a browser. `＋ Wiki` / `＋ GitLab` source buttons.
- [x] **B1 — deterministic grounding-verify (architectural anti-fabrication)** (`notebook_chat.py`) — flags high-specificity verbatim tokens (CVE / @handle / dates / compound identifiers) present in the answer but absent from every retrieved fragment, and appends a neutral caveat — **never rewrites the answer** (prior rewrite-based attempts spawned new hallucination). Bare integers excluded so aggregation counts don't false-fire; dates compared by (year,month,day). 0 false positives on 5 live gemma-26b answers over a technical corpus.
- [x] **Security hardening (adversarial review, 8 confirmed findings fixed)** — SSRF guard (resolve host, reject non-global/loopback/link-local/reserved, re-check every redirect hop); indirect-prompt-injection fencing (`<<<ИСТОЧНИК N>>>` markers + "untrusted data, not instructions" rule in all synthesis prompts); streaming download with a hard size cap; graceful per-source degradation + model warm-up; `certifi`/`httpx` bundled into the frozen exe (the web stack is the first `https://` consumer, so the CA bundle now ships).
