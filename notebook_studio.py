...
MATERIALS: dict[str, MaterialSpec] = {
    ...
    # «Lint» по мотивам LLM-Wiki Карпатого: периодическая проверка здоровья
    # корпуса. Не входит в MATERIAL_ORDER — вызывается отдельной кнопкой.
    "lint": MaterialSpec(
        kind="lint",
        title="Проверка блокнота",
        filename="lint_report.md",
        instructions=(
            "Проведи аудит корпуса как редактор базы знаний. Оформи отчёт в Markdown "
            "с тремя уровнями важности:\n"
            "## 🔴 Противоречия — утверждения из разных мест, которые конфликтуют "
            "(приведи оба и поясни конфликт).\n"
            "## 🟡 Устаревшее и дубликаты — факты, похожие на устаревшие/обновлённые "
            "позже, и явные повторы или дублирующиеся источники.\n"
            "## 🔵 Пробелы и рекомендации — важные темы, упомянутые вскользь без "
            "раскрытия; чего не хватает; что стоит добавить или уточнить.\n"
            "В каждом разделе — маркированный список; если проблем нет, напиши "
            "«— не обнаружено». Опирайся ТОЛЬКО на фрагменты, ничего не выдумывай."
        ),
    ),
}
