...
def build_expansion_messages(
    question: str, n: int = 2, schema: str = ""
) -> list[dict[str, str]]:
    """messages для генерации n перефразировок (модель вернёт JSON-массив строк).

    ``schema`` — описание домена блокнота: с ним перефразировки используют
    лексику корпуса (варианты написания имён/систем, транслитерации, жаргон),
    что напрямую поднимает recall при словарном дрейфе (напр. «кубернетес» в
    вопросе vs «kubernetes» в документах).
    """
    ...
    if s:
        system += (
            "\n\n[Домен базы знаний]\n" + s +
            "\nИспользуй в переформулировках термины и варианты написания из домена."
        )
    user = (
