...
_ANALYTICAL_MARKERS = (
    ...
    "перечисл", "перечень", "список", "какие все", "какие есть", "все связ",
...
_MAP_SYSTEM = (
    ...
    "цель анализа. Выпиши из ЭТОЙ пачки всё, что относится к цели ИЛИ помогает её "
    "понять — прямые факты, КОСВЕННЫЕ признаки, контекст, смежные обстоятельства — "
    "каждое со ссылкой на номер фрагмента [N]. Не выдумывай и не перефразируй "
    "оценочно: опирайся строго на текст, сомнительное помечай как предположение. "
    "Фрагменты с пометкой (контекст) — окружение: используй для понимания, но факт "
    "приписывай тому, кто его сказал/о ком он. "
    "Если в цели указан КОНКРЕТНЫЙ участник (@ник или ФИО), выписывай как факт О НЁМ "
    "ТОЛЬКО то, что сказано ЭТИМ участником или где он ЯВНО фигурирует (его ник/имя "
    "во фрагменте). Активность, интересы и действия из фрагментов, где этого "
    "участника НЕТ (про ботов, устройства, других людей, посторонние темы), НЕ "
    "приписывай ему. "
    "Отвечай ровно НЕТ только если пачка СОВСЕМ не про тему."
)
...
_REDUCE_SYSTEM = (
    ...
    "ВАЖНО про людей: если в тексте стоит @ник — это ИДЕНТИФИКАТОР, а НЕ имя. "
    "Категорически НЕ придумывай к нику имя/фамилию (напр. по @jdoe нельзя писать "
    "«Иван Доу»). Указывай ЛИБО реальное ФИО, встреченное в тексте, ЛИБО сам ник — "
    "но не смешивай разных людей в одно имя. "
    "Если цель — портрет/характеристика КОНКРЕТНОГО участника (@ник/ФИО): приписывай "
    "ему ТОЛЬКО то, что подтверждается наблюдениями, где он сам фигурирует. НЕ "
    "переноси на него активность, интересы и действия из наблюдений, где его нет "
    "(широкий сбор мог захватить чужие темы — боты, устройства, посторонние чаты)."
)

# Промежуточное слияние для иерархического reduce (когда сводок много).
_MERGE_SYSTEM = (
    "Ты объединяешь несколько ЧАСТИЧНЫХ сводок наблюдений (со ссылками [N]) в одну "
    "компактную сводку по цели. Сохрани ВСЕ ссылки [N], убери повторы, ничего не "
    "теряй. Не склеивай разных людей (ник и ФИО могут быть разными людьми). Это "
    "промежуточное слияние — не форматируй как финальный ответ."
)
...
def group_list(items: list, size: int) -> list[list]:
    """Разбить список на группы по ``size`` (для иерархического reduce)."""
    size = max(1, size)
    return [items[i:i + size] for i in range(0, len(items), size)]


def build_merge_messages(goal: str, summaries: list[str]) -> list[dict[str, str]]:
    """Промежуточное слияние группы частичных сводок в одну (сохраняя [N])."""
    joined = "\n\n".join(f"— {s.strip()}" for s in summaries if s.strip())
    user = (
        f"[Цель анализа]\n{goal.strip()}\n\n"
        f"[Частичные сводки]\n{joined}\n\n"
        f"Объедини в ОДНУ компактную сводку по цели, сохрани все ссылки [N], убери повторы."
    )
    return [
